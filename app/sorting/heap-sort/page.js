...
const quizQuestions = [
    {
        ...
        options: ["Binary Search Tree", "Binary Max Heap", "Hash Table", "Linked List"],
        correct: 1,
        explanation: "Heap Sort uses a binary max heap where the largest element is always at the root. This property is used to repeatedly extract the maximum and build the sorted array."
    },
    {
        question: "What is Heap Sort's space complexity?",
        options: ["O(n)", "O(log n)", "O(n log n)", "O(1)"],
        correct: 3,
        explanation: "Heap Sort is in-place — it builds the heap within the original array and doesn't require auxiliary space proportional to the input size, giving O(1) space complexity."
    },
    {
        question: "What are the two main phases of Heap Sort?",
        options: ["Compare and Swap", "Divide and Merge", "Build Max Heap and Extract Elements", "Partition and Recurse"],
        correct: 2,
        explanation: "Phase 1: Build a max heap from the input array. Phase 2: Repeatedly extract the maximum (root) and place it at the end, reducing the heap size each time."
    }
];
...
const HeapSortPage = () => {
    ...
    const generateHeapSortSteps = (arr) => {
        ...
        function heapify(arr, n, i, steps, phase = 'build') {
            ...
            steps.push({
                array: [...tempArr], heapSize: n,
                comparing: [], swapping: [],
                sorted: phase === 'sort' ? Array.from({ length: tempArr.length - n }, (_, idx) => n + idx) : [],
                currentRoot: i, leftChild: left < n ? left : -1, rightChild: right < n ? right : -1,
                phase,
                ...
                heapifyNode: i, maxElement: -1
            });

            if (left < n) {
                steps.push({
                    array: [...tempArr], heapSize: n,
                    comparing: [largest, left], swapping: [],
                    sorted: phase === 'sort' ? Array.from({ length: tempArr.length - n }, (_, idx) => n + idx) : [],
                    currentRoot: i, leftChild: left, rightChild: right < n ? right : -1,
                    phase,
            ...
            if (right < n) {
                steps.push({
                    array: [...tempArr], heapSize: n,
                    comparing: [largest, right], swapping: [],
                    sorted: phase === 'sort' ? Array.from({ length: tempArr.length - n }, (_, idx) => n + idx) : [],
                    currentRoot: i, leftChild: left < n ? left : -1, rightChild: right,
                    phase,
            ...
            if (largest !== i) {
                ...
                steps.push({
                    ...
                    comparing: [], swapping: [],
                    sorted: phase === 'sort' ? Array.from({ length: tempArr.length - n }, (_, idx) => n + idx) : [],
                    currentRoot: largest, leftChild: -1, rightChild: -1,
                    phase,
                    explanation: `Swapped! Continuing to heapify the affected subtree at position ${largest}.`,
                    heapifyNode: largest, maxElement: -1
                });
        ...
        steps.push({
            ...
            currentRoot: -1, leftChild: -1, rightChild: -1,
            phase: 'build_start',
            explanation: `Building Max Heap: Starting from the last non-leaf node (${Math.floor(n / 2) - 1}) and working backwards.`,
            heapifyNode: -1, maxElement: -1
        });
        ...
        steps.push({
            ...
            currentRoot: -1, leftChild: -1, rightChild: -1,
            phase: 'heap_built',
            explanation: `Max Heap built! The largest element ${tempArr[0]} is now at the root. The array represents a complete binary tree with max heap property.`,
            heapifyNode: -1, maxElement: 0
        });

        for (let i = n - 1; i > 0; i--) {
            steps.push({
                array: [...tempArr], heapSize: i + 1,
                comparing: [], swapping: [0, i],
                sorted: Array.from({ length: n - i - 1 }, (_, idx) => i + 1 + idx),
                currentRoot: 0, leftChild: -1, rightChild: -1,
                phase: 'extracting',
            ...
            steps.push({
                array: [...tempArr], heapSize: i,
                comparing: [], swapping: [],
                sorted: Array.from({ length: n - i }, (_, idx) => i + idx),
                currentRoot: 0, leftChild: -1, rightChild: -1,
                phase: 'extracted',
                explanation: `Extracted! ${tempArr[i]} is now in its final sorted position. Heap size reduced to ${i}. Re-heapifying from root.`,
                heapifyNode: 0, maxElement: -1
            });
    ...
    const currentState = stepHistory[currentStep] || {
        array, heapSize: array.length, comparing: [], swapping: [], sorted: [],
        currentRoot: -1, leftChild: -1, rightChild: -1,
        phase: 'start',
        explanation: 'Click Play to begin the Heap Sort visualization',
        heapifyNode: -1, maxElement: -1
    };
    ...
    const getBarColor = (index) => {
        ...
        if (index >= currentState.heapSize) return 'bg-slate-600 border-slate-500';
        ...
        for (let level = 0; level < levels; level++) {
            ...
            for (let i = startIndex; i < endIndex; i++) {
                if (i < heapArray.length) {
                    levelNodes.push({ value: heapArray[i], index: i, color: getBarColor(i) });
                }
            }
            if (levelNodes.length > 0) tree.push(levelNodes);
        }
        return tree;
    };
            ...
            <div className="bg-gradient-to-r from-orange-600 to-amber-600 text-white">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                    ...
                    <div className="text-center">
                        <h1 className="text-4xl md:text-5xl font-bold mb-4">Heap Sort Visualizer</h1>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="bg-slate-900/70 rounded-xl border border-slate-700/50 shadow-xl p-6 mb-6">
                            ...
                            <div className="mb-6">
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <span className="text-sm text-slate-500">Phase: {currentState.phase} | Heap Size: {currentState.heapSize}</span>
                                </div>
                            ...
                            <div className="mb-6">
                                <h3 className="text-sm font-semibold mb-3 text-slate-300">Array Representation</h3>
                            ...
                            {heapTree.length > 0 && (
                                <div className="mb-6">
                                    <h3 className="text-sm font-semibold mb-3 text-slate-300">Binary Heap Tree</h3>
                                    <div className="bg-slate-800/60 rounded-lg border-2 border-slate-700/60 p-4">
                                        {heapTree.map((level, levelIndex) => (
                                            <div key={levelIndex} className="flex justify-center items-center mb-4" style={{ marginLeft: `${(heapTree.length - levelIndex - 1) * 20}px`, marginRight: `${(heapTree.length - levelIndex - 1) * 20}px` }}>
                                                {level.map((node, nodeIndex) => (
                                                    <div key={nodeIndex} className={`w-12 h-12 rounded-full flex items-center justify-center mx-2 text-white font-bold text-sm transition-all duration-500 ${node.color}`}>
                                                        {node.value}
                                                    </div>
                                                ))}
                                            </div>
                            ...
                            <div className="flex flex-wrap justify-center gap-3 mb-6 text-sm">
                                {[['bg-orange-400 border-orange-500', 'Heap Elements'], ['bg-red-500 border-red-600', 'Max Element'], ['bg-blue-500 border-blue-600', 'Current Root'], ['bg-cyan-400 border-cyan-500', 'Child Nodes'], ['bg-yellow-400 border-yellow-500', 'Comparing'], ['bg-purple-500 border-purple-600', 'Swapping'], ['bg-slate-600 border-slate-500', 'Outside Heap'], ['bg-green-500 border-green-600', 'Sorted']].map(([color, label]) => (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-slate-900/70 rounded-xl border border-slate-700/50 shadow-xl p-6">
                            <h3 className="font-bold text-white mb-4">When to Use Heap Sort</h3>
                            <ul className="space-y-2 text-sm text-slate-300">
                                <li className="flex items-start gap-2"><CheckCircle className="h-4 w-4 text-green-500 mt-0.5 flex-shrink-0" /><span>Guaranteed O(n log n) time complexity needed</span></li>
                                <li className="flex items-start gap-2"><CheckCircle className="h-4 w-4 text-green-500 mt-0.5 flex-shrink-0" /><span>Memory-constrained environments (O(1) space)</span></li>
                                <li className="flex items-start gap-2"><CheckCircle className="h-4 w-4 text-green-500 mt-0.5 flex-shrink-0" /><span>Finding k largest/smallest elements</span></li>
                                ...
                                <li className="flex items-start gap-2"><XCircle className="h-4 w-4 text-red-500 mt-0.5 flex-shrink-0" /><span>Small datasets (overhead of heap building)</span></li>
                            </ul>
                        </div>
