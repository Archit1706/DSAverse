...
const quizQuestions = [
    ...
    },
    {
        ...
        correct: 1,
        explanation: "DFS is the foundation for topological sorting, cycle detection, and finding strongly connected components. Its depth-first nature makes it ideal for these structural problems."
    },
    {
        question: "What is DFS's time complexity?",
        options: ["O(V²)", "O(V log V)", "O(V + E)", "O(E log V)"],
        correct: 2,
        explanation: "DFS visits every vertex once and traverses every edge once, giving O(V + E) — same as BFS."
    }
];
...
function generateDFSSteps(startNode, nodes, adj) {
    const steps = [];
    const ns = Object.fromEntries(nodes.map(n => [n.id, 'unvisited']));

    steps.push({ nodeStates: { ...ns }, stack: [], dfsOrder: [], currentNode: -1,
        explanation: `Starting DFS from node ${startNode}. Plunge as deep as possible, backtrack when stuck.` });
    ...
    const dfsVisit = (node) => {
        ...
        steps.push({ nodeStates: { ...ns }, stack: [...callStack], dfsOrder: [...dfsOrder], currentNode: node,
        ...
        for (const nb of (adj[node] || [])) {
            if (!visited.has(nb)) {
                ...
                ns[node] = 'current';
                steps.push({ nodeStates: { ...ns }, stack: [...callStack], dfsOrder: [...dfsOrder], currentNode: node,
                    explanation: `Returned to node ${node} from ${nb}. Checking remaining neighbors.` });
            } else {
                steps.push({ nodeStates: { ...ns }, stack: [...callStack], dfsOrder: [...dfsOrder], currentNode: node,
        ...
        callStack.pop();
        ns[node] = 'visited';
        steps.push({ nodeStates: { ...ns }, stack: [...callStack], dfsOrder: [...dfsOrder], currentNode: -1,
            explanation: `Node ${node} fully explored. Backtracking. Stack: [${callStack.length ? callStack.join(' → ') : 'empty'}]` });
    };

    dfsVisit(startNode);

    steps.push({ nodeStates: { ...ns }, stack: [], dfsOrder: [...dfsOrder], currentNode: -1,
        explanation: `DFS complete! Traversal order: ${dfsOrder.join(' → ')}. All reachable nodes visited.` });
...
export default function DFSPage() {
    ...
    useEffect(() => {
        ...
        setStepHistory(generateDFSSteps(safeStart, nodes, adj));
        setCurrentStep(0);
        setIsPlaying(false);
    ...
    const edgeIsTree = (a, b) => {
        ...
        return (sA === 'visited' || sA === 'current') && (sB === 'visited' || sB === 'current');
    };
                        ...
                        <div className="bg-slate-900/70 rounded-xl border border-slate-700/50 shadow-xl p-6">
                            ...
                            <div className="bg-slate-800/60 rounded-xl border border-slate-700/50 p-4 mb-6">
                                ...
                                <div className="flex flex-wrap justify-center gap-4 mt-3 text-xs text-slate-400">
                                    {[['#334155','#64748b','Unvisited'],['#a16207','#eab308','In Stack'],['#c2410c','#f97316','Current'],['#15803d','#22c55e','Visited']].map(([f,s,l]) => (
                            ...
                            {state.dfsOrder.length > 0 && (
                                <div className="mb-6 text-sm">
                                    <span className="font-semibold text-slate-300">DFS Order: </span>
                                    <span className="text-slate-300">{state.dfsOrder.join(' → ')}</span>
                                </div>
                            )}
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-slate-900/70 rounded-xl border border-slate-700/50 shadow-xl p-6">
                            ...
                            <ul className="space-y-2 text-sm text-slate-300">
                                ...
                                <li className="flex items-start gap-2"><CheckCircle className="h-4 w-4 text-green-500 mt-0.5 flex-shrink-0" /><span>Maze solving and backtracking</span></li>
